...
function FeedCard({m, isOwn, pName, pAvatar, demo, onDelete, onRemove,
  ...
  function openDisputeModal(mode){
    ...
    setDisputeDraft({
      reasonCode:'',
      reasonDetail:'',
      sets:prefill?m.currentProposal.sets.map(function(s){return Object.assign({},s);}):
           m.sets&&m.sets.length?m.sets.map(function(s){return Object.assign({},s);}):
           [{you:'',them:''}],
  ...
  return (
    <div style={{background:t.bgCard,border:"1px solid "+t.border,borderRadius:t.r2,overflow:"hidden",marginBottom:10,opacity:cardOpacity}}>
      {/* Card header */}
      <div style={{padding:"14px 16px 10px",display:"flex",gap:10,alignItems:"center"}}>
        <div style={{width:36,height:36,borderRadius:t.r,flexShrink:0,background:avColor(pName),display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:700,color:"#fff",letterSpacing:"-0.3px"}}>{pAvatar}</div>
        <div style={{flex:1,minWidth:0}}>
          ...
          <div style={{fontSize:10,color:t.textSecondary,marginTop:1,letterSpacing:"0.02em"}}>{m.date}</div>
        </div>
      ...
      <div style={{margin:"0 12px 12px",borderRadius:t.r,border:"1px solid "+(isWin?t.green:t.red)+"28",background:isWin?t.greenSubtle:t.redSubtle,padding:"16px"}}>
        <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:8}}>
          <div>
            <div style={{fontSize:9,fontWeight:700,color:isWin?t.green:t.red,textTransform:"uppercase",letterSpacing:"0.12em",marginBottom:6}}>{isWin?"Victory":"Defeat"}</div>
            <div style={{fontSize:22,fontWeight:700,color:t.text,letterSpacing:"-0.5px",lineHeight:1.1}}>vs {m.oppName}</div>
            {(m.venue||m.court)&&(
              <div style={{fontSize:11,color:t.textSecondary,marginTop:4}}>{[m.venue,m.court].filter(Boolean).join(' · ')}</div>
            )}
          </div>
          {scoreStr&&(
            <div style={{fontSize:28,fontWeight:700,color:isWin?t.green:t.red,fontVariantNumeric:"tabular-nums",letterSpacing:"-1px",lineHeight:1,flexShrink:0}}>{scoreStr}</div>
          )}
        </div>
      </div>
      ...
      {isOpponentView&&!demo&&(
        <div style={{borderTop:"1px solid "+t.border,padding:"12px 16px",display:"flex",gap:8,flexWrap:"wrap"}}>
          <div style={{fontSize:11,color:t.textSecondary,width:"100%",marginBottom:6,fontWeight:500}}>{pName} logged this match — does it look right?</div>
          <button onClick={function(){confirmOpponentMatch(m);}}
            style={{flex:1,padding:"9px 8px",borderRadius:8,border:"none",background:t.green,color:"#fff",fontSize:12,fontWeight:700,minWidth:80}}>✓ Confirm</button>
          <button onClick={function(){openDisputeModal('dispute');}}
            style={{flex:1,padding:"9px 8px",borderRadius:8,border:"1px solid "+t.red+"44",background:t.redSubtle,color:t.red,fontSize:12,fontWeight:600,minWidth:80}}>Dispute</button>
        </div>
      )}
        ...
        <div style={{borderTop:"1px solid "+t.border,padding:"12px 16px"}}>
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            <button onClick={function(){acceptCorrection(m);}}
              style={{flex:1,padding:"9px 8px",borderRadius:8,border:"none",background:t.green,color:"#fff",fontSize:12,fontWeight:700,minWidth:80}}>Accept</button>
            {(m.revisionCount||0)<3&&(
              <button onClick={function(){openDisputeModal('counter');}}
                style={{flex:1,padding:"9px 8px",borderRadius:8,border:"1px solid "+t.orange,background:t.orangeSubtle,color:t.orange,fontSize:12,fontWeight:600,minWidth:80}}>Counter-propose</button>
            )}
            <button onClick={function(){if(window.confirm("Void this match? This cannot be undone."))voidMatchAction(m,"mutual_void");}}
              style={{flex:1,padding:"9px 8px",borderRadius:8,border:"1px solid "+t.red+"44",background:t.redSubtle,color:t.red,fontSize:12,fontWeight:600,minWidth:80}}>Void</button>
          </div>
        </div>
      )}
    ...
    return (
      <div style={{maxWidth:680,margin:"0 auto"}}>
        <div style={{padding:"28px 20px 20px"}}>
          ...
          <div style={{display:"flex",gap:10,marginBottom:28}}>
            <button onClick={function(){setShowAuth(true);setAuthMode("signup");setAuthStep("choose");}}
              style={{flex:1,padding:"13px",borderRadius:9,border:"none",background:t.accent,color:"#fff",fontSize:14,fontWeight:700}}>Join free</button>
            <button onClick={function(){setShowAuth(true);setAuthMode("login");setAuthStep("choose");}}
              style={{flex:1,padding:"13px",borderRadius:9,border:"1px solid "+t.border,background:"transparent",color:t.text,fontSize:14,fontWeight:500}}>Log in</button>
          </div>
        </div>
        <div style={{position:"relative",padding:"0 20px 40px"}}>
          ...
          <div style={{position:"absolute",top:"40%",left:"50%",transform:"translate(-50%,-50%)",textAlign:"center",background:t.bgCard,border:"1px solid "+t.border,borderRadius:14,padding:"20px 24px",boxShadow:"0 8px 32px rgba(0,0,0,0.08)",width:"calc(100% - 80px)",maxWidth:320}}>
            ...
            <button onClick={function(){setShowAuth(true);setAuthMode("signup");setAuthStep("choose");}}
              style={{width:"100%",padding:"12px",borderRadius:8,border:"none",background:t.accent,color:"#fff",fontSize:13,fontWeight:700}}>Get started</button>
          </div>
        </div>
      </div>
    );
      ...
      <div style={{padding:"0 20px 100px"}}>
        {history.length===0
          ?<div style={{background:t.bgCard,border:"1px solid "+t.border,borderRadius:12,padding:"40px 24px",textAlign:"center"}}>
            ...
            <div style={{fontSize:13,color:t.textSecondary,lineHeight:1.6,marginBottom:24}}>Log your first match and it'll show up in your feed.</div>
            <button onClick={openLogMatch} style={{padding:"13px 28px",borderRadius:9,border:"none",background:t.accent,color:"#fff",fontSize:14,fontWeight:700}}>Log your first match</button>
          </div>
