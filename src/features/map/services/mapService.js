...
export async function fetchPlayersAtCourt(courtName, viewer, limit, excludeIds) {
  ...
  var selfReq = supabase.from("profiles")
    .select("id,name,avatar,avatar_url,skill,suburb,home_zone,availability,played_courts,ranking_points,last_active,show_online_status,show_last_seen,gender,birth_year")
...
export function fetchPlayersInZone(zoneId, limit, excludeIds){
  ...
  var q = supabase.from("profiles")
    .select("id,name,avatar,avatar_url,suburb,skill,ranking_points,last_active,home_zone,gender,birth_year");
