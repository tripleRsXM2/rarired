...
- v10 — Player Trust & Reliability foundation (2026-04-30, Module 10 Slice 1). New `player_trust_profiles` (server-owned aggregate cache) + `post_match_feedback` (private user feedback) tables. Trust ≠ Reliability ≠ Confidence — three concepts kept separate. New users default to `trust=50, reliability=50, confidence=low` (neutral, never suspicious). `submit_post_match_feedback` SECURITY DEFINER RPC is the ONLY insert path for feedback (eligibility-gated, status must be confirmed/pending_reconfirmation, opponent must be linked, no self-review, UNIQUE blocks duplicates). `recalculate_player_trust_profile(uuid)` is fully derived from source-of-truth tables on every call — no permanent mutations, "one-time penalty" means condition applied in current recalc. AFTER triggers on match_history / challenges / match_invites / post_match_feedback fan recalc out, all wrapped in EXCEPTION-WHEN-OTHERS so trust math can never block a match-truth write. **Confirmed-match trust contribution capped at +30** (+2 each for first 10, +1 thereafter). **Repeated-opponent farming** demotes confidence by one tier only — does NOT directly subtract trust_score, does NOT affect Elo. **Anti-retaliation**: feedback weight 0.5× while dispute open, 0× on voided matches, 0.75× on confirmed-after-dispute, 1.0× on clean. Distinct-reporter threshold (≥ 2) ensures one disputed negative report can never alone trigger a public penalty. Public surface = positive-only badge (`new` / `building` / `responsive` / `reliable` / `confirmed`) via `player_trust_public` view; numerical scores never exposed to other users. **No Elo interaction in V1.** Slice 2 (UI + Discover boost) is the next module. Full spec: `docs/player-trust-and-reliability.md`.
- v9 — Opponent-invite flow (2026-04-28, Module 9). New match status `pending_opponent_claim` precedes `pending_confirmation` for matches against not-yet-on-CourtSync opponents. Token-based share link (server-generated, SHA-256 hashed, 30-day expiry, single-use); SECURITY DEFINER `claim_match_invite` is the only path that can attach `opponent_id` and flip status to `pending_confirmation`. **Pre-claim matches do NOT affect**: CourtSync Rating, displayed skill level, calibration progress, `confirmed_ranked_match_count`, competitive league standings, or verified-match count. After claim + opponent confirmation, the standard `apply_match_outcome` rules apply unchanged (one-set 0.60× / BO3 weight table / opponent-strength asymmetry / etc.). `validate_match_score` BEFORE-INSERT trigger now applies its strict rules to `pending_opponent_claim` rows too. Full spec: `docs/opponent-invite-flow.md`.
