...
- v4 — **Trust + Reliability ranking signal (2026-04-30, Module 10 Slice 1, schema-only)**. `player_trust_profiles` and the public-readable `player_trust_public` view now provide derived reliability signals (`public_badge`, `confidence_level`, response/completion rates, distinct-reporter counts) that Discovery and the challenge composer can use to rank candidates. **Slice 1 ships the data model only — no UI wiring yet.** Slice 2 will: (a) update `fetchSuggestedPlayers` to join `player_trust_public` and order candidates by reliability score (server-side via a new RPC, never exposing the numeric score to the client), (b) apply a confidence floor so only `medium`/`high` users get the boost (new players still surface — they just don't outrank reliable ones with similar skill), (c) add a subtle positive tag in the challenge composer ("Usually responds" / "Confirmed player") if the target qualifies. Low-reliability users are ranked-down, never hidden. No public negative tags. Full spec: `docs/player-trust-and-reliability.md`.
- v3 — **Opponent invite as activation channel (2026-04-28, Module 9)**. Every freetext-opponent match is now a potential user-acquisition surface: the logger generates a server-signed share-link the opponent uses to sign in / sign up + claim. Pre-claim matches don't affect rating, calibration, or league standings — the trust gate stays pristine until the recipient explicitly confirms. Hypothesis: invite-claim conversion becomes the dominant top-of-funnel acquisition signal at seed scale. A single seed user with 5 active opponents generates 5 acquisition opportunities per month at no marketing cost. Tracked events: `opponent_invite_created` / `opponent_invite_shared` (with channel) / `opponent_invite_opened` / `opponent_invite_claimed` / `opponent_invite_declined`. Full spec: `docs/opponent-invite-flow.md`.
