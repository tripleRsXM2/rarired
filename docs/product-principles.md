...
- v8 — **Design overhaul (2026-04-25)**: locked in three product lenses extending the existing principles into visual + structural rules — (1) identity over utility, (2) one thing matters most per screen, (3) calm hierarchy beats density. Home stops being feed-as-home and becomes a composite landing experience: Hero + Next Action + Leagues + condensed Feed. North star "my tennis world, calm and clear." Borrows rhythm from Nike Run Club; explicitly avoids Strava-style equal-weight density. Full spec in `docs/design-direction.md`. Slice 1 = Home; subsequent slices = Profile, Log Match, Leagues, Feed polish.
- v9 — **CourtSync Rating naming rule (2026-04-27)**: the rating number is now formally called "CourtSync Rating" in user-facing copy. The product **never** claims to be UTR, USTA, NTRP, or any official tennis-federation ranking — that rule was already implicit in the "no external rating import" item under "What we are NOT building", but the explicit rename ships with Module 7.7. Internal language ("Elo", "K-factor", "expected score") is fine in code + docs; user-facing language is "CourtSync Rating", "calibration", "provisional / established". Full spec in `docs/trust-and-ranking-rules.md` v8.
