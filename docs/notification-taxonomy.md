...
| `message_request` | action | Non-friend sends a 1:1 DM (groups bypass the request gate — see Groups in `messaging.md`) | Target | `View message →` opens conversation |
| `request_accepted` | important | Target accepts your friend request | Original sender | `View profile →` — opens `/profile/:from_user_id` |
| `message_request_accepted` | important | Target accepts your 1:1 DM request (group conversations skip this flow) | Original sender | `View message →` |
...
| `group_added` | important | `create_group_conversation` RPC fans out one row per non-creator participant the moment the group is materialised — server-side, inside the SECURITY DEFINER RPC | Each non-creator participant | `Open group →` opens the conversation by `entity_id` |
...
| `league_invite` | important | Owner invites you to a league via `invite_to_league` RPC | Invited user | `View invite →` (deep-links to `/tournaments/leagues?id=<league>`) |
| `league_joined` | important | Member accepts an invite (fires to existing active members) | Active members | `View league →` |
| `league_completed` | important | Owner runs `complete_league` — final standings locked | Active members (excl. owner) | `View league →` (in-app only — no push in V1) |
| `league_archived` | important | Owner runs `archive_league` — league moves off the active list, standings frozen | Active members (excl. owner) | `View league →` (in-app only — no push in V1) |
...
- v12 — **Group conversations (phase 4, 2026-04-27)**. `message_request` and `message_request_accepted` are explicitly scoped to **1:1 only** — group conversations bypass the request gate (added participants join immediately, no pending state). The doubles-invite flow can refuse to create a group when any pair of intended participants currently blocks each other; surfaced via the refusal copy below.
- v13 — **`group_added` notification (2026-05-01)**. New informational type fired by `create_group_conversation` to every non-creator participant the moment the group is materialised. Closes a UX gap: previously B and C only saw a notification once A sent the first message, so a freshly-created group could sit silent in their inbox for minutes. Insert happens server-side inside the SECURITY DEFINER RPC (one row per non-creator), so the frontend has no fan-out responsibility. Fields: `entity_id` = conv id (drives the "Open group →" deep-link), `metadata.participant_ids` = full participant set incl. creator, `metadata.participant_count` = full size. Push category is `null` (in-app only) — the first DM in the group already pushes via the `message_request` / message-family flows, so a second push at create-time would double-tap. Mirror obligation: the type is registered in `src/features/notifications/types.js` and labelled in `notifUtils.getNotifLabel`. Server-side server fan-out bypasses `emit_notification` (the RPC has already validated the participant + block sets as a unit, and routing through emit would force per-recipient round-trips we don't want).

### Refusal copy

When `create_group_conversation` rejects a doubles invite due to a block conflict, the UI shows:

> **"That group can't be created right now. Try messaging them individually instead."**

Single line, no jargon ("blocked", "conflict") so neither the blocker nor the blocked party can infer the other's state. The fallback CTA is opening the singles 1:1 message flow, where the existing block check applies normally per partner.

- v11 — **League lifecycle notifications (Module 12 Slice 2, 2026-04-27)**. Four new types added to round out the league lifecycle state machine landed in Slice 1: `league_completed`, `league_archived`, `league_cancelled`, `league_voided`. All four are non-action ("important" category — informational, surfaceable but non-blocking). Fan-out happens server-side inside the SECURITY DEFINER lifecycle RPCs (`complete_league` / `archive_league` / `cancel_league` / `void_league`) via the new `_emit_league_lifecycle_notifs(league_id, type, actor, metadata)` helper, which writes one row per active member except the actor themselves. The same RPCs also call `_resolve_pending_league_invites(league_id)` so any pending `league_invite` notifications for the league are marked `resolved_at = now()` — the league is no longer joinable, the invite is moot. Bypasses `emit_notification` (already validated owner standing inside the RPC; lifecycle is one-shot and not subject to dedup). `league_voided` deliberately renders no in-app CTA: the league has been filtered out of the recipient's normal lists (app-level `useLeagues` filter), so a "View league" deep-link would dead-end. The notif body is the only signal — without it members would silently lose the league with no explanation. Notif copy reads the league name from `metadata.league_name` (set by the RPC) and falls back to "your" when missing. **Push policy (V1):** all four lifecycle types are **in-app only** — `push_category = null` in `src/features/notifications/types.js`, absent from `notificationService.PUSH_WORTHY_TYPES`, and intentionally NOT in the Edge Function's `PUSH_TYPE_TO_CATEGORY`. Lifecycle events on a private league don't earn lock-screen real estate yet, and `league_voided` in particular shouldn't ping every member's home screen on what's typically an owner cleanup move. Re-enable per-type if a retention argument calls for it. Mirror obligation: any new lifecycle type must update both this file's table AND `src/features/notifications/types.js` AND `notificationService.PUSH_WORTHY_TYPES` AND the Edge Function's category/template maps in lockstep — the registry is split across four surfaces (tracked as a Module 12 follow-up).
