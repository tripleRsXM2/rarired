...
| `league_completed` | important | Owner runs `complete_league` — final standings locked | Active members (excl. owner) | `View league →` |
| `league_archived` | important | Owner runs `archive_league` — league moves off the active list, standings frozen | Active members (excl. owner) | `View league →` |
| `league_cancelled` | important | Owner runs `cancel_league` — season ended without a final table | Active members (excl. owner) | `View league →` |
| `league_voided` | important | Owner runs `void_league` — wrong setup / test data; league disappears from members' lists | Active members (excl. owner) | (no CTA — league is gone) |
