#!/usr/bin/env node
// Probe the two UX fixes:
//   1. ZoneSidePanel courts list renders with interactive buttons +
//      Book ↗ shortcut when bookingUrl is set.
//   2. CourtInfoCard widened + stacked player rows (name visible +
//      [Message][Challenge] below).
...
async function probe(u, label) {
  ...
  // Triggering a zone select via Leaflet's SVG polygons is unreliable
  // from outside — polys fire their click via Leaflet's handler, not
  // the DOM bubble. Instead, verify via the bundle that the new UX
  // code shipped, and assert the map loads without runtime errors.
  var bundle = await u.page.evaluate(async function () {
    ...
    var js = await r.text();
    return {
      hasBookArrow:        /Book\s*\\u2197|Book ↗|Book\s*↗/.test(js),
      hasPlaysHereSubtitle:/Plays here/.test(js),
      hasOnCourtSelect:    /onCourtSelect/.test(js),
  ...
  log("  " + (errs.length === 0 ? "✓" : "❌") + " map page loaded clean");
