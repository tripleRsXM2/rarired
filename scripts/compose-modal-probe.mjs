#!/usr/bin/env node
// Probe the inline ComposeMessageModal. Bundle inspection +
// runtime-error check on both accounts. Clicking through to the full
// modal requires tapping a Leaflet SVG polygon which is unreliable in
// headless; we trust the wiring + bundle symbols instead.
...
async function probe(u, label) {
  ...
  var snap = await u.page.evaluate(async function () {
    ...
    var js = await r.text();
    return {
      hasComposeHeader:  /New message to/.test(js),
      hasToneLabel:      /\bTone\b/.test(js) && /Casual/.test(js),
      hasSendButton:     /Sending…|Or open the conversation/.test(js),
      hasDmSentFromMap:  /dm_sent_from_map/.test(js),
