#!/usr/bin/env node
// Probe the three map trims:
//   - Zone numbers gone from labels
//   - Home badge is a circle with a home icon
//   - Court markers are clustered at low zoom (visible cluster
//     elements painted by the markercluster plugin)
...
async function probe(u, label) {
  ...
  var snap = await u.page.evaluate(function () {
    // Cluster markers — leaflet.markercluster paints divs with class
    // `marker-cluster` (`marker-cluster-small/medium/large`). At the
    // initial zoom we should have several.
    var clusters = Array.from(document.querySelectorAll(".marker-cluster, .cs-court-cluster"));
    var leafletMarkers = Array.from(document.querySelectorAll(".leaflet-marker-icon"));

    // Zone label numbers — used to render "1 / 2 / 3 …" inside the
    // colored circles. Should now be 0.
    var labelText = document.querySelectorAll(".cs-zone-label")[0];
    var labels = Array.from(document.querySelectorAll(".cs-zone-label"));
    var labelInner = labels.map(function (el) { return (el.innerText || "").trim(); }).join(" | ");

    // Home circle — should appear when viewer has a home zone set
    // (we're signed in, so this depends on the user). Detect by
    // looking for a div with the new home-glyph SVG path.
    var hasHomeCircle = false;
    labels.forEach(function (el) {
      if (/M3 8l6-5 6 5/.test(el.innerHTML || "")) hasHomeCircle = true;
    });

    return {
      url: location.pathname,
      clusterCount: clusters.length,
      totalMarkers: leafletMarkers.length,
      labelTextSample: labelInner.slice(0, 200),
      hasHomeCircle: hasHomeCircle,
      // Specifically look for any "1.", "2.", "3." preface that the
      // old badge used — should be empty now.
      hasOldZoneNumber: /(^|\s)[1-6][.\s]/.test(labelInner),
    };
  });
